...
import { SkPolars, SkConversions } from './sk-data.js';
...
export default class SkClient extends EventEmitter {
    ...
    reconnect() {
        ...
        */
        ...
        if (this.socket !== null) {
            this.debug('[reconnect] closing socket')
            this.socket.close()
            return
        }
        ...
        if (this.options.reconnect === false) {
            this.debug('[reconnect] Not reconnecting, for reconnect is false')
            this.cleanupListeners()
            return
        }

        if (this.shouldDisconnect === true) {
            this.debug('[reconnect] not reconnecting, shouldDisconnect is true')
            this.cleanupListeners()
            return
        }

        this.debug(`[reconnect] socket is ${this.socket === null ? '' : 'not '}NULL`)
        ...
        return this.fetch('/auth/login', authRequest)
            .then(result => {
                ...
                this.debug(`[reconnect] successful auth request: ${JSON.stringify(result, null, 2)}`)
            ...
            .catch(err => {
                this.debug(`[reconnect] error logging in: ${err.message}, reconnecting`)
                this.emit('error', err)
                this._retries += 1
                this.isConnecting = false
    ...
    initiateSocket() {
        this.debug(`[initiateSocket] retry counter: ${this._retries}/${this.options.maxRetries}`)
        this.socket = this.websocket_factory(this.wsURI);
    ...
    cleanupListeners() {
        this.debug(`[cleanupListeners] resetting auth and removing listeners`)
        ...
        if (typeof this.state !== "undefined") {
            if (typeof data.updates !== "undefined") {
                for (const update of data.updates) {
                    if (typeof update.values !== "undefined") {
    ...
    _onWSOpen() {
        this.debug('[_onWSOpen] called with wsURI:', this.wsURI)
        this._connection = null
        this.connected = true
        this.isConnecting = false
        this._retries = 0
    ...
    _onWSError(err) {
        this.debug('[_onWSError] WS error', err.message || '')
        this._connection = null
        this._retries += 1
        this.emit('error', err)
        this.reconnect()
    ...
    _onWSClose(evt) {
        this.debug('[_onWSClose] called with wsURI:', this.wsURI)
    ...
    send(data) {
        ...
        if (typeof data === 'string') {
            try {
                ...
                this.debug(`[send] data is string but not valid JSON: ${e.message}`)
            }
        }
        ...
        this.debug(`Sending data to socket: ${data}`)
    ...
    fetch(path, opts) {
        ...
        if (this._authenticated === true && !path.includes('auth/login')) {
            ...
            this.debug(`[fetch] enriching fetch options with in-memory token`)
        }
        ...
        this.debug(`[fetch] ${opts.method || 'GET'} ${URI} ${JSON.stringify(opts, null, 2)}`)
